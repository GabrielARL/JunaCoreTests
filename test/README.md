...
| `synthetic-channel` | `synthetic_uwa_channel.jl` | A deterministic six-tap/per-path-Doppler channel and calibrated seeded AWGN drive matched-FEC waterfalls through all six modules. Payloads, channel taps, Doppler, SNR, and noise seeds are shared; the frame-wide modem uses its required global-code waveform. A separate fixture checks coupled-state discrimination under physical model mismatch. |
...
| `frame-wide-ldpc` | `frame_wide_ldpc.jl` | Every frame modem uses one systematic LDPC codeword and one parity graph across all OFDM blocks. Five executable frame receiver profiles isolate Standard FFT, Partial-FFT pilot-band LS, Lite posterior refitting, global W,z refinement, and global W,C,z refinement; the legacy stateful band-RLS profile remains the paper-reproduction default. The suite pins profile dispatch, cross-block parity, exact clean recovery, invalid-profile behavior, a hand-computed two-step RLS recurrence, inherited-state behavior, Rpchan pilots, and realized `d_c`. |
...
| `noise` | `noise_roundtrip.jl` | All public receivers consume the same 12 seeded payloads and unit complex-noise vectors. Per-symbol receivers also share identical clean waveforms; Frame-wide LDPC uses its matched global code. Ordinary codes are bracketed clean at 4 dB and the sparse frame code at 7 dB; all retain `0 < BER < 0.30` at 1.5 dB, `BER > 0.05` at 0 dB, and monotone BER in SNR. |
