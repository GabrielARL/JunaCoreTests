...
| `commit-rate-benchmark` | `commit_code_rate_benchmark.jl` | The commit-history benchmark holds SG-1, 20 dB, seed 1, CP=16, and one OFDM block fixed while sweeping N=512/1024/2048 and LDPC rates 1/16, 1/8, 1/4, and 1/2 across Standard, Partial FFT, Lite, Wz, and WCz. The clean identity contract proves all 60 geometries are valid and payload-exact, including capacity-fitted coded lengths 672/1360/2720; `tools/record_commit_code_rate_benchmark.jl` performs the measured run, rejects dirty JunaCore source, and records PSR/BER/decode time under the exact source commit. |
