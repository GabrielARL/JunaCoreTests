...
| True frame-wide benchmark contract only | `julia --project=. test/runtests.jl commit-frame-benchmark` |
| Record full-capture SG-1 frame table | `make record-frame-benchmark` |
...
| `layout` | `ofdm_layout.jl` | DC-nulled carrier plan: 341 comb pilots (agrees with fig caption, 672) + data tones partition the active set; deterministic ±1 pilot signs (golden prefix); contiguous near-even band partition (`eq:ofdm5-band-index`, 885–889). `bw*fs` is the occupied RF width and integer `dc0` shifts its centre from `fc` in kHz; the test pins 18--30 kHz for `fc=fs=24 kHz, bw=0.5, dc0=0` and 19--31 kHz for `dc0=1`, within one FFT bin while keeping DC nulled. |
| `frequency-geometry` | `rf_frequency_geometry.jl` | FFTs the waveform produced by every public module and proves its nonzero carrier bins realize the declared RF band. All seven defaults occupy approximately 12--36 kHz for `fc=fs=24 kHz`; the six configurable modules roundtrip at 18--30 kHz (`bw=0.5, dc0=0`) and 19--31 kHz (`dc0=1`). FrameRLS directly proves its pinned full-band waveform and explicit rejection of unsupported half-band/shifted configurations. |
| `ldpc` | `ldpc_code_construction.jl` | LDPC helpers resolve from `tools/ldpc`, contain no developer-home path, and build from a clean working directory with range-safe deterministic seeds. The default k=340, n=1360 `evencol` code, generated `H`, generator rows, Tanner adjacency, cache invalidation, and `H·codeword == 0 mod 2` are checked before BP/JUNA consume the graph. Impossible dimensions/degrees fail before invoking a helper, and helper-process failures become contextual `ArgumentError`s at the modem boundary. |
...
| `sync` | `sync_waveform.jl` | Optional LFM sync is deterministic and isolated from coarse Doppler: `sync=false` is a no-op, `sync=true` emits a 2048-sample unit-magnitude LFM waveform, matched correlation peaks at the embedded lag, and every LFM-capable public receiver wraps OFDM blocks as `[sync][blocks][sync]`. JUNA-FrameRLS is intentionally N/A because its identity pins the separate Rpchan QPSK preamble. |
...
| `synthetic-channel` | `synthetic_uwa_channel.jl` | A deterministic six-tap/per-path-Doppler channel and calibrated seeded AWGN drive matched-FEC waterfalls through the six native JunaCore modes. Payloads, channel taps, Doppler, SNR, and noise seeds are shared; the frame-wide modem exercises the same stateful band-RLS algorithm used by JUNA-FrameRLS. The pinned Rpchan preset is N/A for this independently-Doppler-scaled synthetic protocol and is instead judged by acquisition, golden-frame, and measured replay gates. |
...
| `frame-wide-ldpc` | `frame_wide_ldpc.jl` | Every frame modem uses one systematic LDPC codeword and one parity graph across all OFDM blocks. Five executable frame receiver profiles isolate Standard FFT, Partial-FFT pilot-band LS, Lite posterior refitting, global W,z refinement, and global W,C,z refinement. `JunaFrameRLS.Modulation` exposes the stateful band-RLS paper receiver as a pinned public preset. The suite pins profile dispatch, cross-block parity, exact clean recovery, invalid-profile behavior, a hand-computed two-step RLS recurrence, inherited-state behavior, Rpchan pilots, acquisition, and realized `d_c`. |
...
| `noise` | `noise_roundtrip.jl` | All seven public receivers consume the same 12 seeded payloads and prefixes of the same unit complex-noise vectors. Native per-symbol receivers share identical clean waveforms; frame-wide and JUNA-FrameRLS paths retain their own matched FEC and framing overhead. Ordinary codes are bracketed clean at 4 dB and sparse frame codes at 7 dB; all retain `0 < BER < 0.30` at 1.5 dB, `BER > 0.05` at 0 dB, and monotone BER in SNR. |
...
| `golden-diff` | `rpchan_golden_comparison.jl` | A forcing check requires the comparator to enter through `JunaCore.JunaFrameRLS.Modulation`. The synthetic contract proves exact-input comparison, active-bin/`1/sqrt(N)` Partial-FFT normalization, seed/final stage separation, first-divergence short-circuiting, and explicit LDPC structural mismatch reporting. With `JUNA_RPCHAN_GOLDEN=1`, ReplayCh exports one deterministic SG-1 receiver-3 frame at 20 dB and the public JUNA-FrameRLS path consumes the exact passband samples stage by stage. |
| `receiver-benchmark` | `receiver_channel_benchmark.jl` | The common `/benchmark` page compares the five receiver-only profiles on identical per-symbol-coded samples across all 12 captures. Frame-wide LDPC remains N/A in that table because it changes transmit FEC framing. Its matched-transmitter lanes pin the paper configurations and native rates, use the Rpchan-compatible preamble, carrier order, passband replay, global systematic LDPC code, and re-encoded per-packet acceptance rule. The SG-1 five-profile baseline additionally proves that one generated/noised sample vector feeds Standard, PFFT, Lite, Wz, and WCz, and reports packet PSR separately from exact ten-block frame PSR. |
| `commit-rate-benchmark` | `commit_code_rate_benchmark.jl` | The commit-history benchmark holds SG-1, 20 dB, seed 1, and CP=16 fixed while sweeping N=512/1024/2048, LDPC rates 1/16, 1/8, 1/4, and 1/2, and requested total pilot ratios 0.25, 0.50, and 0.75 across Standard, Partial FFT, Lite, Wz, and WCz. Each pilot budget is split equally between inner and outer pilots, then snapped to the realizable comb spacings 8, 4, and 3. The clean identity contract proves all 180 geometries are valid and payload-exact, including capacity-fitted coded lengths 880/1776/3568, 752/1520/3056, and 672/1360/2720. The measured recorder then maps sequential complete blocks over more than 99% of the full 47.7917-second SG-1 capture, rejects dirty JunaCore source, and records block PSR, aggregate mean BER, mean decode time per block, and successful-block effective data rate under the exact source commit. |
| `commit-frame-benchmark` | `commit_frame_benchmark.jl` | The matching true frame-wide benchmark sweeps the same 36 SG-1 geometries and five named algorithms, but one LDPC codeword and parity graph span ten consecutive OFDM blocks. Its clean contract validates all 180 cells, shared frame payload accounting, profile dispatch, exact recovery, and rejected invalid frame geometry. The full-capture recorder logs frame PSR, aggregate BER, mean decode time per ten-block frame, and successful-frame effective data rate under a clean JunaCore source commit. |
...
| `contract` | `interface_contract.jl` | The shared catalog must exactly equal all seven public runtime modes. Every modem has an executable capability; `JunaFrameRLS.Modulation` additionally declares and executes `:frame_stateful_band_rls`, pins its identity fields and SG-1 defaults, permits compatible channel geometries, and passes clean acquired interface and two-block frame roundtrips. The opt-in strict gate exercises three-block framing for all seven public modules. |
