...
const SUITES = [
    ...
    (key = "synthetic-channel", file = "synthetic_uwa_channel.jl",
     ...
     claim = "the six-tap paper PDP, full linear convolution, per-path time scale and carrier Doppler, and calibrated seeded AWGN form a replay-data-free physical channel gate; receivers share payload/channel/noise realizations while Frame-wide LDPC uses its matched FEC waveform, and a second fixture proves band-tied C still favors truth under model mismatch",
    ...
    (key = "frame-wide-ldpc", file = "frame_wide_ldpc.jl",
     title = "Five receiver algorithms over one frame-wide LDPC graph",
     claim = "Standard, Partial-FFT, Lite, Wz, and WCz frame profiles decode one sparse systematic (Bk,Bn) graph spanning all OFDM blocks; the legacy Rpchan-compatible stateful band-RLS/JUNA profile remains available while ordinary receivers retain per-symbol codes",
